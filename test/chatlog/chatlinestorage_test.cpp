...
{
    QCOMPARE(initialStartIdx, 10);
    QCOMPARE(initialEndIdx, 20);
    QCOMPARE(storage.size(), 10);

    storage.erase(ChatLogIdx(11));

    QCOMPARE(storage.size(), 9);

    QCOMPARE(idxFromChatLine(storage[0]).get(), 10);
    QCOMPARE(idxFromChatLine(storage[1]).get(), 12);
